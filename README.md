flowchart TB
  ...
  %% ---------- Legend ----------
  subgraph legend["Legend / Rules of thumb"]
    legend_service[Touches Firestore → service]
    legend_provider[Manages in-memory state → provider]
    legend_widget[Renders UI → widget]
    legend_page[Orchestrates screen flow → page]
    legend_model[Defines data shape → model]
  end
  ...
  intro_page --> libProviders
  video_call_page --> libProviders
  steps_page --> libProviders
  ...
  section_card --> pagesCore
  adaptive_app_bar --> pagesCore
  adaptive_bottom_nav --> pagesCore
  unit_icon --> pagesCore

  %% Models feed Providers/Services/UI
  exModels --> exProviders
  exModels --> exServices
